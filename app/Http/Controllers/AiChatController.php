...
class AiChatController extends Controller
{
    ...
    public function ask(Request $request)
    {
        ...
        $articles = (!$isGreetingOnly && $this->shouldRecommendArticles($message, $intent))
            ...
            : collect();
        $history = $session->messages()->oldest()->take(10)->get();
        ...
        $quickChoices = $this->buildQuickChoices($message, $answer, $intent, $products, $articles);
    ...
    {
        $providers = [
            ['name' => 'gemini', 'runner' => fn () => $this->askGemini($message, $intentLabel, $products, $articles, $history, $userName)],
            ['name' => 'groq', 'runner' => fn () => $this->askGroq($message, $intentLabel, $products, $articles, $history, $userName)],
            ['name' => 'openrouter', 'runner' => fn () => $this->askOpenRouter($message, $intentLabel, $products, $articles, $history, $userName)],
        ];
        ...
        foreach ($providers as $provider) {
            ...
            try {
                $answer = ($provider['runner'])();
                $duration = (int) round((microtime(true) - $startedAt) * 1000);
                if ($answer) {
                    $this->logAiProvider($userId, $provider['name'], 'success', $duration, $message, null);
                    $notice = null;
                    if ($provider['name'] === 'groq') {
                        $notice = 'Catatan: Gemini sedang tidak tersedia/limit, jadi aku pakai AI cadangan Groq agar jawaban tetap pintar.';
                    } elseif ($provider['name'] === 'openrouter') {
                        $notice = 'Catatan: Gemini/Groq sedang tidak tersedia atau limit, jadi aku pakai AI cadangan OpenRouter.';
                    }
                    return [$answer, $provider['name'], $notice];
                }
                $failed[] = $provider['name'];
                $this->logAiProvider($userId, $provider['name'], 'failed_or_empty', $duration, $message, null);
            } catch (\Throwable $e) {
                $duration = (int) round((microtime(true) - $startedAt) * 1000);
                $failed[] = $provider['name'];
                $this->logAiProvider($userId, $provider['name'], 'exception', $duration, $message, $e->getMessage());
            }
        }

        $notice = null;
        if (config('services.gemini.api_key') || env('GEMINI_API_KEY')) {
            $notice = 'Catatan: Semua provider AI sedang tidak tersedia/limit, jadi aku pakai jawaban cadangan lokal dulu.';
        }
        return [null, 'local_fallback', $notice];
    }
    ...
    private function askGroq(string $message, string $intentLabel, $products, $articles, $history, string $userName): ?string
    {
        ...
        $systemPrompt = "Kamu AI SobatAnak, asisten Mom & Baby Care berbahasa Indonesia. Jawab pertanyaan user secara langsung, detail, natural, dan praktis. Jangan memaksa rekomendasi produk/artikel kalau tidak relevan. Untuk kesehatan anak, beri edukasi umum aman, bukan diagnosis atau dosis obat. Sebut nama user jika cocok: {$userName}.";
        $userPrompt = "Pertanyaan user: {$message}\nIntent: {$intentLabel}\nRiwayat:\n{$historyText}\nProduk relevan jika perlu:\n{$productContext}\nArtikel relevan jika perlu:\n{$articleContext}\n\nBerikan jawaban yang spesifik dan tidak mengulang template.";
...
PROMPT;
    ...
    private function shouldRecommendArticles(string $message, string $intent): bool
    {
        ...
        $q = Str::lower($message);
        $articleKeywords = [
            'artikel','bacaan','panduan','tips','cara','kenapa','mengapa','bagaimana','solusi',
        ...
        return in_array($intent, ['perawatan','edukasi','newborn','pakaian','nutrisi','stroller','kesehatan','parenting','keuangan'], true);
    }
    ...
    private function shouldRecommendProducts(string $message, string $intent): bool
    {
        ...
        $q = Str::lower($message);
        $shoppingKeywords = [
            'produk','barang','beli','belanja','rekomendasi','rekomendasikan','pilih','cari',
            'botol','popok','mainan','lotion','sabun','shampoo','sampo','baju','pakaian',
            'stroller','mpasi','sereal','perlengkapan','perawatan','anti kolik','newborn'
        ];

        if (in_array($intent, ['perawatan','edukasi','newborn','pakaian','nutrisi','stroller'], true)) {
            return true;
        }
    ...
    private function searchProducts(string $message, string $intent)
    {
        ...
        return $query->orderByDesc('rating')->orderByDesc('sold')->take(4)->get();
    }
