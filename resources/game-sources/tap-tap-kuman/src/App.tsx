...
export default function App() {
  ...
  const hasReportedScoreRef = useRef<boolean>(false);
  ...
  useEffect(() => {
  const bgm = new Audio(`${import.meta.env.BASE_URL}audio/backsound-tap-kuman.mp3`);
  bgm.loop = true;
  bgm.volume = 0.25;
...
const spawnKuman = useCallback(() => {
  ...
  const reportScoreToSobatAnak = useCallback(async (finalScore: number) => {
    if (hasReportedScoreRef.current) return;
    hasReportedScoreRef.current = true;

    const token = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
    setPointStatus('Menyimpan poin ke akun SobatAnak...');
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        if (data.redirect) {
          setPointStatus('Login dulu agar poin game tersimpan ke akun kamu.');
          return;
        }
        throw new Error(data.message || 'Poin belum berhasil tersimpan.');
      }

      setEarnedPoints(Number(data.earned || 0));
      setPointStatus(data.message || 'Poin berhasil tersimpan.');
      document.querySelectorAll('[data-points], [data-points-bottom]').forEach((el) => {
        el.textContent = Number(data.points || 0).toLocaleString('id-ID');
      });
    } catch (error) {
      console.warn(error);
      setPointStatus('Poin belum tersimpan. Coba main lagi setelah login.');
    }
  }, []);
  ...
  useEffect(() => {
    if (gameState === 'FINISHED') {
      reportScoreToSobatAnak(score);
      if (score > highScore) {
        setHighScore(score);
        localStorage.setItem("taptap_high_score", score.toString());
      }
    }
  }, [gameState, score, highScore, reportScoreToSobatAnak]);
...
/>
        ...
        {gameState === 'FINISHED' && (
          <div className="relative z-10 p-4 md:p-6 text-center max-w-3xl flex flex-col items-center justify-center animate-pop-in w-full">
            ...
            <p className="text-base md:text-xl font-black italic text-slate-600 mb-4 text-center">
              "Luar biasa! Kamu telah menjadi pahlawan kebersihan hari ini!"
            </p>

            <div className="bg-white border-4 border-emerald-100 rounded-3xl px-5 py-4 shadow-lg mb-6 md:mb-8 max-w-xl w-full">
              <p className="text-emerald-700 font-black text-base md:text-lg">
                {earnedPoints !== null ? `+${earnedPoints} poin berhasil ditambahkan` : pointStatus || 'Skor sedang diproses menjadi poin SobatAnak.'}
              </p>
              <p className="text-slate-500 font-bold text-xs md:text-sm mt-1">
                Poin hanya tersimpan otomatis kalau kamu sudah login.
              </p>
            </div>
