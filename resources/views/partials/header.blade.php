    ...
    <div class="site-header-inner max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center gap-4">
        ...
        @if($authUser)
            <div class="relative profile-menu">
                <button data-profile-toggle class="profile-btn"><span class="profile-avatar">{{ strtoupper(substr($authUser->name,0,1)) }}</span><span class="hidden lg:block text-left leading-tight"><b>{{ $authUser->name }}</b><br><small>Profile</small></span></button>
        ...
        @else
            <div class="relative profile-menu">
                <button data-profile-toggle class="w-11 h-11 rounded-full border-2 border-[#D4EEEC] bg-[#E8F5F4] flex items-center justify-center text-[#2A3D3C] hover:bg-[#D0F0ED] hover:border-[#4BBFB0] transition-all duration-200" title="Akun Saya">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" class="w-5 h-5 text-[#2A3D3C]">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                    </svg>
                </button>
                <div data-profile-menu class="profile-dropdown">
                    <a href="{{ route('login') }}">🔑 Login / Masuk</a>
                    <a href="{{ route('register') }}">📝 Register / Daftar</a>
                </div>
            </div>
        @endif
