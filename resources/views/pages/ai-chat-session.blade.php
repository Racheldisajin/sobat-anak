...
<section class="gemini-page">
    <aside class="gemini-sidebar">
        ...
        {{-- User profile card di bottom sidebar --}}
        <div class="gemini-sidebar-user">
            @php
                $avatarPath = $authUser?->avatar ?? null;
                $avatarUrl  = $avatarPath
                    ? ((str_starts_with($avatarPath,'http')||str_starts_with($avatarPath,'/')) ? $avatarPath : asset($avatarPath))
                    : null;
                $initial = strtoupper(substr($authUser?->name ?? 'U', 0, 1));
            @endphp
            <div class="gemini-sidebar-avatar">
                @if($avatarUrl)
            ...
            <div class="gemini-sidebar-info">
                <b>{{ $authUser?->name ?? 'Pengguna' }}</b>
                <small>{{ $authUser?->email ?? '' }}</small>
            </div>
            <a href="{{ route('profile') }}" class="gemini-sidebar-profile-btn" title="Lihat Profile">⚙</a>
        </div>
    </aside>
                            ...
                            @if($m->role === 'assistant' && $storedArticles->count())
                                <div class="gemini-rec-wrap">
                                    ...
                                    @foreach($storedArticles as $article)
                                        <a class="gemini-article-card" href="{{ url('/artikel/' . ($article->slug ?: $article->id)) }}">
...
.gemini-page{min-height:100vh;background:linear-gradient(135deg,#fffaf3 0%,#effdf9 48%,#fff4f1 100%);color:var(--sa-text);display:grid;grid-template-columns:minmax(260px,300px) minmax(0,1fr);font-family:'Baloo 2',system-ui,sans-serif;overflow-x:hidden}.gemini-page *{box-sizing:border-box}
...
.gemini-sidebar{display:flex;flex-direction:column}
.gemini-recent-list{flex:1;overflow-y:auto;overflow-x:hidden}
.gemini-sidebar-user{margin-top:auto;padding:.75rem .6rem .6rem;border-top:1px solid var(--sa-line);display:flex;align-items:center;gap:.65rem;background:linear-gradient(180deg,transparent,rgba(75,191,176,.06));border-radius:0 0 0 1.2rem}
.gemini-sidebar-avatar{width:2.2rem;height:2.2rem;border-radius:999px;background:linear-gradient(135deg,#4BBFB0,#E8756A);display:flex;align-items:center;justify-content:center;flex:0 0 auto;overflow:hidden;box-shadow:0 4px 12px rgba(75,191,176,.3)}
.gemini-sidebar-avatar img{width:100%;height:100%;object-fit:cover}
.gemini-sidebar-avatar span{color:#fff;font-weight:900;font-size:.9rem;font-family:'Fredoka',system-ui,sans-serif}
.gemini-sidebar-info{flex:1;min-width:0}
.gemini-sidebar-info b{display:block;font-size:.82rem;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--sa-text)}
.gemini-sidebar-info small{display:block;font-size:.7rem;color:var(--sa-muted,#6b8a88);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.gemini-sidebar-profile-btn{flex:0 0 auto;width:1.8rem;height:1.8rem;border-radius:999px;background:#f0fbf9;border:1px solid var(--sa-line);display:flex;align-items:center;justify-content:center;font-size:.85rem;color:#4BBFB0;text-decoration:none;transition:.2s}
.gemini-sidebar-profile-btn:hover{background:#4BBFB0;color:#fff}

/* ── Cute Custom Dialog & Toast ── */
.sa-dialog-overlay{position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;background:rgba(42,61,60,.35);backdrop-filter:blur(4px);animation:saDialogIn .2s ease}
@keyframes saDialogIn{from{opacity:0}to{opacity:1}}
.sa-dialog-box{background:#fff;border-radius:1.8rem;padding:2rem 1.8rem 1.5rem;max-width:320px;width:90%;text-align:center;box-shadow:0 28px 60px rgba(42,61,60,.18);border:1.5px solid rgba(75,191,176,.25);animation:saBoxIn .25s cubic-bezier(.2,.8,.2,1)}
@keyframes saBoxIn{from{opacity:0;transform:scale(.88) translateY(12px)}to{opacity:1;transform:scale(1) translateY(0)}}
.sa-dialog-icon{font-size:2.6rem;margin-bottom:.5rem;display:block;animation:saBounceIcon .5s ease .1s both}
@keyframes saBounceIcon{0%{transform:scale(0) rotate(-15deg)}60%{transform:scale(1.18) rotate(5deg)}100%{transform:scale(1) rotate(0)}}
.sa-dialog-title{font-family:'Fredoka',system-ui,sans-serif;font-size:1.25rem;font-weight:700;color:#2a3d3c;margin:.2rem 0 .5rem}
.sa-dialog-desc{font-size:.9rem;color:#6b8a88;font-weight:700;line-height:1.55;margin-bottom:1.2rem}
.sa-dialog-actions{display:flex;gap:.7rem;justify-content:center}
.sa-dialog-btn{border:0;border-radius:999px;padding:.7rem 1.4rem;font-weight:900;font-size:.9rem;cursor:pointer;transition:.2s;font-family:'Baloo 2',system-ui,sans-serif}
.sa-dialog-btn:hover{transform:translateY(-2px)}
.sa-dialog-btn.confirm{background:#E8756A;color:#fff;box-shadow:0 8px 20px rgba(232,117,106,.3)}
.sa-dialog-btn.confirm:hover{background:#d05a50}
.sa-dialog-btn.cancel{background:#f0fbf9;color:#4BBFB0;border:1.5px solid rgba(75,191,176,.35)}
.sa-dialog-btn.cancel:hover{background:#d0f0ed}
/* Toast */
.sa-toast-container{position:fixed;top:1.2rem;right:1.2rem;z-index:99999;display:flex;flex-direction:column;gap:.6rem;pointer-events:none}
.sa-toast{background:#fff;border:1.5px solid rgba(75,191,176,.3);border-radius:1.2rem;padding:.8rem 1.1rem;display:flex;align-items:center;gap:.65rem;box-shadow:0 18px 40px rgba(42,61,60,.12);pointer-events:auto;animation:saToastIn .3s cubic-bezier(.2,.8,.2,1);min-width:220px;max-width:300px}
@keyframes saToastIn{from{opacity:0;transform:translateX(30px)}to{opacity:1;transform:translateX(0)}}
.sa-toast.leaving{animation:saToastOut .28s ease forwards}
@keyframes saToastOut{to{opacity:0;transform:translateX(30px)}}
.sa-toast-icon{font-size:1.35rem;flex:0 0 auto}
.sa-toast-text{flex:1;font-weight:800;font-size:.88rem;color:#2a3d3c;line-height:1.4}
.sa-toast.success .sa-toast-icon{filter:none}
.sa-toast.error{border-color:rgba(232,117,106,.35)}
</style>

{{-- Toast Container (di luar section agar tidak ter-clip) --}}
<div class="sa-toast-container" id="saToastContainer"></div>

<script>
// ── Cute Dialog & Toast helpers ──
function saConfirm({icon='🗑️', title='Yakin nih?', desc='', confirmText='Hapus 💨', cancelText='Batal 🙈', onConfirm}){
    const overlay = document.createElement('div');
    overlay.className = 'sa-dialog-overlay';
    overlay.innerHTML = `<div class="sa-dialog-box">
        <span class="sa-dialog-icon">${icon}</span>
        <div class="sa-dialog-title">${title}</div>
        ${desc ? `<div class="sa-dialog-desc">${desc}</div>` : ''}
        <div class="sa-dialog-actions">
            <button class="sa-dialog-btn cancel">${cancelText}</button>
            <button class="sa-dialog-btn confirm">${confirmText}</button>
        </div>
    </div>`;
    document.body.appendChild(overlay);
    overlay.querySelector('.cancel').onclick = () => overlay.remove();
    overlay.querySelector('.confirm').onclick = () => { overlay.remove(); onConfirm && onConfirm(); };
    overlay.addEventListener('click', e => { if(e.target === overlay) overlay.remove(); });
}
function saToast(text, type='success'){
    const container = document.getElementById('saToastContainer');
    if(!container) return;
    const icons = {success:'✅', error:'😢', info:'💬'};
    const toast = document.createElement('div');
    toast.className = `sa-toast ${type}`;
    toast.innerHTML = `<span class="sa-toast-icon">${icons[type]||'✨'}</span><span class="sa-toast-text">${text}</span>`;
...
(() => {
    ...
    @php
        $userAvatarPath = $authUser?->avatar ?? null;
        $userAvatarUrl  = $userAvatarPath
            ? ((str_starts_with($userAvatarPath,'http')||str_starts_with($userAvatarPath,'/')) ? $userAvatarPath : asset($userAvatarPath))
            : null;
    @endphp
    const userAvatarUrl = @json($userAvatarUrl);
    ...
    function addMessage(role, html){
        ...
        const userAvatarHtml = userAvatarUrl
            ? `<div class="gemini-avatar" style="padding:0;overflow:hidden"><img src="${userAvatarUrl}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:999px"></div>`
            : `<div class="gemini-avatar">${esc(userInitial)}</div>`;
        wrap.innerHTML = role === 'user'
            ? `${userAvatarHtml}<div class="gemini-bubble">${html}</div>`
            : `<div class="gemini-avatar ai-logo"><img src="{{ asset('assets/logo-sobat-anak-bw.png') }}" alt="SobatAnak AI"></div><div class="gemini-bubble">${html}</div>`;
    ...
    async function ask(text){
        ...
        try {
            ...
            let html = esc(data.answer || 'Maaf, AI belum memberi jawaban.').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>');
            ...
            if (data.quick_choices && data.quick_choices.length) html += choiceButtons(data.quick_choices);
            botBubble.innerHTML = html;
            scrollBottom();
        } catch (e) {
            console.error('AI chat error:', e);
            const errMsg = (e && e.message && e.message !== 'Failed to fetch')
                ? esc(e.message)
                : 'Koneksi ke AI terputus sebentar. Coba kirim ulang pesan ya~ 🐣';
            botBubble.innerHTML = `<span style="color:#e8756a">😅 ${errMsg}</span>`;
        } finally {
            sending = false;
    ...
    if (recentList) {
        recentList.addEventListener('submit', async (e) => {
            ...
            saConfirm({
                ...
                desc: 'Chat ini akan hilang selamanya ya~ Yakin hapus?',
                confirmText: 'Hapus 💨',
                cancelText: 'Batal 🙈',
                onConfirm: async () => {
                    try {
                        ...
                        if (!response.ok || data.ok === false) throw new Error(data.message || 'Gagal menghapus.');
                        ...
                        saToast('Percakapan berhasil dihapus! 🎉', 'success');
                        if (isActive) setTimeout(() => { window.location.href = data.redirect || '{{ route('ai-chat.page') }}'; }, 800);
                    } catch (err) {
                        saToast('Aduh, gagal hapus nih 😢 Coba refresh dulu ya!', 'error');
                    }
                }
            });
        });
