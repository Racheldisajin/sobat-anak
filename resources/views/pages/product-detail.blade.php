...
<div class="max-w-7xl mx-auto px-4 md:px-8 py-4">
    ...
    <div class="mt-10">
        ...
        @if(session('user_id'))
            ...
            @else
            <div class="pdp-review-form-box mt-5" id="my-review-tools">
                <div id="my-review-summary" class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                        <p class="text-teal font-black">✓ Anda sudah memberikan ulasan untuk produk ini.</p>
                        <p class="text-xs text-[#6B8A88] font-bold mt-1">Kamu masih bisa mengedit rating/pesan atau menghapus ulasanmu.</p>
                    </div>
                    <div class="flex gap-2 flex-wrap">
                        <button type="button" onclick="openEditReview()" class="pdp-mini-btn pdp-mini-btn-edit">✏️ Edit Ulasan</button>
                        <button type="button" onclick="deleteReview({{ $product->id }})" class="pdp-mini-btn pdp-mini-btn-delete">🗑️ Hapus</button>
                    </div>
                </div>

                <div id="my-review-edit-form" class="hidden mt-4 border-t border-[#D4EEEC] pt-4">
                    ...
                    <p class="text-xs text-[#6B8A88] font-bold mt-2">Setelah disimpan, ulasan akan diberi label <b>Edited</b>.</p>
                </div>
            </div>
            @endif
        @else
        ...
        <div id="pdp-reviews-list" class="mt-5 grid gap-4">
            ...
            @php
                $reviewAvatarPath = $r->user->avatar ?? null;
                $reviewAvatarUrl = $reviewAvatarPath ? ((str_starts_with($reviewAvatarPath, 'http://') || str_starts_with($reviewAvatarPath, 'https://') || str_starts_with($reviewAvatarPath, '/')) ? $reviewAvatarPath : asset($reviewAvatarPath)) : null;
                $reviewIsAdmin = strtolower((string)($r->user->role ?? 'user')) === 'admin';
            ...
            <div class="pdp-review-card" id="review-{{ $r->id }}">
                <div class="flex items-center gap-3 mb-2">
                    <div class="pdp-review-avatar">
                        @if($reviewAvatarUrl)
                            <img src="{{ $reviewAvatarUrl }}" alt="{{ $r->user->name ?? 'Pengguna' }}">
                        @else
                            {{ strtoupper(substr($r->user->name ?? 'U', 0, 1)) }}
                        @endif
                    </div>
                    <div>
                        <p class="font-black text-sm flex items-center gap-2 flex-wrap">{{ $r->user->name ?? 'Pengguna' }} @if($reviewIsAdmin)<span class="admin-review-badge">Admin SobatAnak</span>@endif</p>
                        <p class="text-xs text-[#6B8A88]">
...
<style>
.pdp-review-avatar{overflow:hidden}
.pdp-review-avatar img{width:100%;height:100%;border-radius:inherit;object-fit:cover;display:block}
.admin-review-badge{display:inline-flex;align-items:center;width:max-content;border:1.5px solid #F7C34A;background:linear-gradient(135deg,#FFF7D9,#FFE7A6);color:#9A5B00;border-radius:999px;padding:.18rem .55rem;font-size:.66rem;font-weight:1000;text-transform:uppercase;letter-spacing:.08em;font-style:normal}
</style>
<script>
...
async function submitReview(productId) {
    ...
    if (res.ok) {
        ...
        card.innerHTML = `
            <div class="flex items-center gap-3 mb-2">
                <div class="pdp-review-avatar">${res.review.user_avatar ? `<img src="${res.review.user_avatar}" alt="${res.review.user_name}">` : res.review.user_name.charAt(0).toUpperCase()}</div>
                <div>
                    <p class="font-black text-sm flex items-center gap-2 flex-wrap">${res.review.user_name} ${res.review.user_role === 'admin' ? '<span class="admin-review-badge">Admin SobatAnak</span>' : ''}</p>
...
function openEditReview() {
    document.getElementById('my-review-summary')?.classList.add('hidden');
    document.getElementById('my-review-edit-form')?.classList.remove('hidden');
    setEditReviewStar(editRating || 5);
    document.getElementById('my-review-tools')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
function cancelEditReview() {
    document.getElementById('my-review-edit-form')?.classList.add('hidden');
    document.getElementById('my-review-summary')?.classList.remove('hidden');
}
...
async function updateReview(productId) {
    ...
    const res = await postJson(`/products/${productId}/reviews`, { rating: editRating, body });
    if (!res) return;
    if (res.ok) {
        showToast('✅ ' + res.message, 'success');
